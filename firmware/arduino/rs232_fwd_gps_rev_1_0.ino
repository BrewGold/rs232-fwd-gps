...
bool parseNmeaDouble(const char* field, double* value) {
  ...
  if (endPtr == field || *endPtr != '\0' || !isfinite(parsed)) return false;
  *value = parsed;
  return true;
}
...
void parseGGA(const char* line) {
  ...
  if (parseNmeaDouble(fields[8], &parsedHdop) && parsedHdop >= 0.0) {
    ...
    strncpy(hdopField, "1.0", sizeof(hdopField));
    hdopField[sizeof(hdopField) - 1] = '\0';
  }
  ...
  while (Serial1.available()) {
    ...
    if (c == '$' || c == '#') {
      ...
      if (c == '\n') {
        ...
        if (strncmp(gnssLineBuf, "$GPGGA", 6) == 0 ||
            strncmp(gnssLineBuf, "$GNGGA", 6) == 0 ||
            strncmp(gnssLineBuf, "$GCGGA", 6) == 0) {
